...
def save_misclassified_samples(dataloader, model, classes):
    model.eval()
    ...
    for idx in range(num_images):
        ...
        for i in range(start_idx, end_idx):
            ...
            ax.axis('off')
            ax.set_title(
                f"Predicted: {classes[predicted_label]}, True: {classes[true_label]}")

        plt.tight_layout()

        # 디렉토리가 존재하지 않으면 생성
        os.makedirs('./output/missclassified', exist_ok=True)

        # 이미지 파일로 저장
        plt.savefig(os.path.join('./output/missclassified',
                    f'misclassified_samples_subplot_{idx}.png'))
        plt.close()
...
plt.plot([e.cpu().detach().numpy()
         for e in error_train], 'black', label='cost function')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
...
# save_misclassified_samples(test_dataloader, model, classes)
